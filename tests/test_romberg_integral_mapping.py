...
from gmapi.mappings.helperfuns import numeric_jacobian, romberg_integral_propagate
...
class TestRombergIntegralJacobian(unittest.TestCase):
    ...
    def generate_myintfun(self, xref, interp, rtol):
        def myintfun(y):
            ...
            return np.array([romberg_integral_propagate(xref, tmpfun,
                                                        maxord=18, rtol=rtol)])
        return myintfun
    ...
    def test_compare_romberg_integral_jacobian_with_numerical_jacobian(self):
        ...
        for rtol in [1e-3, 1e-5, 1e-7]:
            for interp in possible_interp:
                ...
                Stest = romberg_integral_propagate(xref, myfun, dfun=mydfun, maxord=18, rtol=rtol)
    ...
    def test_permuted_vs_original_input_for_romberg_integral_jacobian(self):
        ...
        res1 = romberg_integral_propagate(xref1, myfun1, dfun=mydfun1, maxord=14)
        res2 = romberg_integral_propagate(xref2, myfun2, dfun=mydfun2, maxord=14)
