...
mod_dt = red_dt[red_dt.NODE.str.match('xsid_')]
exp_dt = red_dt[red_dt.NODE.str.match('exp_')]
exp_dt.groupby('REAC')['REAC'].count()
exp_dt.groupby('REAC')['ENERGY'].max()

red_exp_dt = exp_dt.loc[red_dt.REAC=='MT:1-R1:9']
red_mod_dt = mod_dt.loc[red_dt.REAC=='MT:1-R1:9']
...
propfun, prop_logpdf = postdist.generate_proposal_fun(mh_startvals, scale=0.07, rho=0.5)
...
plt.plot(np.arange(smpl.shape[1]), smpl[-1, :])
plt.show()

meanvec = np.mean(smpl[:, 500:], axis=1)
stdvec = np.std(smpl[:, 500:], axis=1)
meanvec[-10:]
stdvec[-10:]
