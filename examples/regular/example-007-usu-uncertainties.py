...
cdt = attach_endep_usu_df(dt, ['MT:1-R1:9'], [12.5, 17.5], [0.1]*2)
...
mh_res['accept_rate']
smpl = mh_res['samples']
prior_reac_idcs = priordt.index[priordt.NODE.str.match('xsid_9')]

plt.plot(np.arange(len(mh_res['logprob_hist'])), mh_res['logprob_hist'])
plt.show()

# red_smpl = smpl[prior_reac_idcs[0], 5000:]
red_smpl = smpl[-2, 100:]
plt.plot(np.arange(len(red_smpl)), red_smpl)
plt.show()

covmat = np.cov(smpl[-2:, 5000:])
stdvec = np.sqrt(covmat.diagonal())
cormat = covmat / (stdvec.reshape(1, -1) * stdvec.reshape(-1, 1) )
