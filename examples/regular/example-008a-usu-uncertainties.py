...
cdt = dt.copy()
cdt = attach_endep_usu_df(cdt, ['MT:1-R1:8'], [0, 7, 12.5, 20], [0.1]*4)
cdt = attach_endep_usu_df(cdt, ['MT:1-R1:9'], [0, 7, 12.5, 20], [0.1]*4)
cdt = attach_endep_usu_df(cdt, ['MT:1-R1:10'], [0, 7, 12.5, 20], [0.1]*4)
...
plt.plot(np.arange(smpl.shape[1]), smpl[-3, :])
plt.show()
...
priordt['RELUNC'] =  stdvec[:-len(postdist._groups)] / meanvec[:-len(postdist._groups)]
priordt[priordt.NODE.str.match('xsid_9') & (priordt.ENERGY > 1) & (priordt.ENERGY < 20)]


plt.hist(smpl[-2, 3000:])
plt.show()
